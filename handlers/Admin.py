...
async def start(message: types.Message):
    if message.from_user.id in config.admins:
        await message.answer(texts.admin.start, parse_mode="HTML",
...
async def statistick(call: types.CallbackQuery):
    await call.message.edit_text(texts.admin.statistick(int(database.count())),
...
async def mailing2(message, state):
    ...
    await message.answer(
        f'Рассылка успешно завершена: {c} / {database.count()}',
        reply_markup=AdminPanel)
    await state.finish()
